...
export default function GitHubActivity() {
  const [contributions, setContributions] = useState<
    Array<{ date: string; level: number; count: number }>
  >([]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          {stats.map((stat, index) => (
            ...
            >
              <div
                className={`inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-700/50 mb-4 group-hover:scale-110 transition-transform duration-200`}
              >
                ...
                {contributions.map((contrib, index) => (
                  <div
                    ...
                    className={`w-3 h-3 rounded-sm ${getContributionColor(
                      contrib.level
                    )} hover:ring-1 hover:ring-green-400 transition-all duration-200 cursor-pointer`}
